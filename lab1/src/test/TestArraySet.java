...
public class TestArraySet {
	...
	public final void testAddAll() {
		...
		s2.add(5);
		s.addAll(s2);
		assertTrue("Does not contain 2.", s.contains(2));
		assertTrue("Does not contain 5.", s.contains(5));
	}
	
	@Test
	public final void testAddAllDuplicate() {
		s.add(2);
		int size = s.size();
		ArraySet<Integer> s2 = new ArraySet<Integer>();
		s2.add(2);
		assertFalse("Should not add 2", s.addAll(s2));
		assertEquals("Wrong size", size, s.size());
	}
	
	@Test
	public final void testAddAllEmpty() {
		ArraySet<Integer> s2 = new ArraySet<Integer>();
		assertFalse("List was modified from an empty list.", s.addAll(s2));
	}
}
